...
# 2. Top Genres by Average Rating (Bar Chart)
genre_stats = (df.explode('genre')
                .groupby('genre')
                .agg({'imdb_rating': ['mean', 'count']})
                .droplevel(0, axis=1))
genre_stats = genre_stats[genre_stats['count'] > 5]  # Filter niche genres
...
plt.xlim(7, 9)  # Focus on meaningful range
plt.grid(axis='x', alpha=0.3)
plt.tight_layout()
plt.show()

# 3. Ratings Over Time (Line Plot with Trendline)
plt.figure(figsize=(12, 6))
...
# --- CLEANER GENRE VISUALIZATION ---

# Step 1: Calculate genre stats (filter genres with at least 5 movies)
genre_stats = (df.explode('genre')
               .groupby('genre')
               .agg(avg_rating=('imdb_rating', 'mean'),
                    movie_count=('imdb_rating', 'count'))
               .query('movie_count >= 5')  # Filter niche genres
...
# Step 3: Add value labels and customize
plt.title('Top Genres by Average IMDb Rating (Min 5 Movies)', 
          fontsize=14, pad=20)
plt.xlabel('Average Rating', fontsize=12)
plt.xlim(7.5, 9.0)  # Focus on meaningful range
...
# Explode genres and calculate stats
genre_stats = (df.explode('genre')
               .groupby('genre')
               .agg(avg_rating=('imdb_rating', 'mean'),
                    movie_count=('imdb_rating', 'count'))
               .sort_values('avg_rating', ascending=True))  # Sort for horizontal bars
